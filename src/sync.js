...
class SqlSync {
  ...
  constructor(model_type, options) {
    ...
    for (let key in options) { const value = options[key]; this[key] = value; }
  ...
  connect(url) {
    ...
    if (this.slaves != null ? this.slaves.length : undefined) {
      for (let slave_url of Array.from(this.slaves)) { this.connections.slaves.push(new Connection(`${slave_url}/${this.table}`)); }
    }
