...
module.exports = (SqlCursor = (function() {
  SqlCursor = class SqlCursor extends sync.Cursor {
    constructor(...args) {
      {
      ...
      this.execUnique = this.execUnique.bind(this);
    ...
    processResponse(json, ast, callback) {
      ...
      for (let model_json of Array.from(json)) { this.backbone_adapter.nativeToAttributes(model_json, schema); }
    ...
    fetchIncludedRelations(json, ast, callback) {
      ...
      return relation_query.asCallback((err, raw_relation_json) => {
        ...
        for (var placeholder of Array.from(relation_json)) {
    ...
    unjoinResults(raw_json, ast) {
      ...
      for (let row of Array.from(raw_json)) {
        var found, related_json, relation_key;
        var model_json = {};
