...
(function() {
  ...
  ref = BackboneORM = require('backbone-orm'), Backbone = ref.Backbone, Queue = ref.Queue, Schema = ref.Schema, Utils = ref.Utils, JSONUtils = ref.JSONUtils, DatabaseURL = ref.DatabaseURL;
  ...
  SqlSync = (function() {
    ...
    SqlSync.prototype.create = function(model, options) {
      var json, save_json;
      json = model.toJSON();
      save_json = this.parseJSON(json);
      return this.getTable('master').insert(save_json, 'id').asCallback((function(_this) {
        return function(err, res) {
          ...
          json.id = res[0];
          return options.success(_this.backbone_adapter.nativeToAttributes(json, _this.schema));
        };
      })(this));
    };
    ...
    SqlSync.prototype.update = function(model, options) {
      ...
      json = model.toJSON();
      save_json = this.parseJSON(json);
      return this.getTable('master').where('id', model.id).update(save_json).asCallback((function(_this) {
        return function(err, res) {
          if (err) {
            return options.error(err);
          }
    ...
    SqlSync.prototype.deleteCB = function(model, callback) {
      return this.getTable('master').where('id', model.id).del().asCallback((function(_this) {
        return function(err, res) {
          if (err) {
            return callback(err);
          }
    ...
    SqlSync.prototype.parseJSON = function(_json) {
      var json, key, ref1, ref2, ref3, value;
      json = _.clone(_json);
      ref1 = this.schema.fields;
      for (key in ref1) {
        value = ref1[key];
        if (((ref2 = (ref3 = value.type) != null ? ref3.toLowerCase() : void 0) === 'json' || ref2 === 'jsonb') && json[key]) {
