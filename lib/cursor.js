...
(function() {
  var COMPARATORS, COMPARATOR_KEYS, Knex, SqlAst, SqlCursor, _, _appendCondition, _appendConditionalWhere, _appendLimits, _appendSelect, _appendSort, _appendWhere, _appendWhereAst, _columnName, _extractCount, _parseSortField, _prefixColumns, _tablePrefix, buildQueryFromAst, ref, sync,
  ...
  _appendWhere = function(query, conditions, table) {
    ...
    if (conditions.or_wheres.length) {
      query.where(function() {
        var j, len1, nested_query, ref2, results;
        nested_query = this;
  ...
  _tablePrefix = function(model_type) {
    return (model_type.tableName()) + "_";
  };

  _prefixColumns = function(model_type, fields) {
    var col, columns;
    ...
    return (function() {
      ...
      for (i = 0, len = columns.length; i < len; i++) {
        col = columns[i];
        results.push((model_type.tableName()) + "." + col + " as " + (_tablePrefix(model_type)) + col);
      }
      return results;
    })();
  };

  buildQueryFromAst = function(query, ast) {
  ...
  module.exports = SqlCursor = (function(superClass) {
    ...
    SqlCursor.prototype.queryToJSON = function(callback) {
      var $columns, ast, err, error, from_columns, i, key, len, query, query_old, ref1, related_model_type, related_wheres, relation, to_columns;
      ...
      if (this._cursor.$include) {
        ...
        from_columns = this._prefixColumns(this.model_type, ast.fields);
      ...
      if (true) {
        ...
        $columns || ($columns = this.joined ? this._prefixColumns(this.model_type, ast.fields) : this._columns(this.model_type, ast.fields));
