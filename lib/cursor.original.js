...
(function() {
  var Ast, Knex, SqlCursor, Utils, _, buildQueryFromAst, extractCount, ref, sync,
  ...
  module.exports = SqlCursor = (function(superClass) {
    ...
    SqlCursor.prototype.execUnique = function(callback) {
      var ast, err, i, len, query, rank_field, raw_query, ref1, ref2, ref3, sort, subquery;
      try {
        ast = new Ast({
          find: this._find,
          cursor: this._cursor,
        ...
        if (this._cursor.$count) {
          ...
          return query.asCallback((function(_this) {
        ...
        if (_.difference(ast.select, this._cursor.$unique).length === 0) {
          ...
          raw_query = (ast.select.map((function(_this) {
            return function(s) {
              return "\"" + s + "\"";
            };
          })(this)).join(', ')) + ", rank() over (partition by \"" + rank_field + "\"";
          if ((ref1 = ast.sort) != null ? ref1.length : void 0) {
            if (sort = (ref2 = ast.sort) != null ? ref2.shift() : void 0) {
          ...
          raw_query += ")";
          subquery = this.connection.select(this.connection.raw(raw_query));
    ...
    SqlCursor.prototype.queryToJSON = function(callback) {
      ...
      this.verbose || (this.verbose = this._cursor.$verbose);
      if (this.verbose) {
    ...
    SqlCursor.prototype.runQuery = function(query, ast, _callback) {
      var callback;
      callback = _callback;
      if (this.verbose) {
        ...
        callback = (function(_this) {
          return function(err, res) {
            console.log('Query complete in', new Date().getTime() - _this.start_time, 'ms');
    ...
    SqlCursor.prototype.fetchIncludedRelations = function(json, ast, callback) {
      ...
      return relation_query.asCallback((function(_this) {
    ...
    SqlCursor.prototype.selectResults = function(json) {
      var $select, $values, field, item, key;
      if (this._cursor.$one) {
        json = json.slice(0, 1);
      }
      if (this._cursor.$values) {
        ...
        $select = (function() {
          var i, len, ref1, results;
          ref1 = this._cursor.$select;
          results = [];
          for (i = 0, len = ref1.length; i < len; i++) {
            field = ref1[i];
            results.push(indexOf.call(field, '.') >= 0 ? field.split('.').pop() : field);
          }
          return results;
        }).call(this);
        if (this._cursor.$whitelist) {
          $select = _.intersection($select, this._cursor.$whitelist);
        }
        json = (function() {
          var i, len, results;
          results = [];
