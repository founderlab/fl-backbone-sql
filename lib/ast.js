  ...
  module.exports = SqlAst = (function() {
    ...
    SqlAst.prototype.parseQuery = function(query, options) {
      ...
      for (key in query) {
        ...
        if (key.indexOf('.') > 0) {
          if (cond = this.parseJsonField(key, value)) {
            conditions.push(cond);
          } else {
            cond = this.parseDotRelation(key, value);
            conditions.push(cond);
          }
    ...
    SqlAst.prototype.parseDotRelation = function(key, value) {
      var cond, current_model_type, current_relation, current_relation_key, relation_field, relation_keys;
      relation_keys = key.split('.');
      relation_field = relation_keys.pop();
      current_model_type = this.model_type;
      while (relation_keys.length) {
        current_relation_key = relation_keys.shift();
        current_relation = this.getRelation(current_relation_key, current_model_type);
        this.join(current_relation_key, current_relation);
        current_model_type = current_relation.reverse_model_type;
      }
      cond = this.parseCondition(relation_field, value, {
        related: true,
